...
accumulator that propagates from the grid centre outward---replacing
...
\begin{enumerate}
\item A complete integer-only rotation algorithm parameterised by
...
\subsection{Integer parameterisation}
...
distance from the grid centre.  Rather than multiplying, we propagate
...
\State $\mathrm{acc} \gets 0$; \; $\mathrm{total} \gets 0$;
  \; $\mathrm{shifts}[\mathrm{centre}] \gets 0$
\For{$d = 1, 2, \ldots, L-1$}
  ...
  \EndWhile
  \State $\mathrm{shifts}[\mathrm{centre}+d] \gets
...
distance~$d$ reads its neighbour at distance~$d-1$ and applies one
Bresenham step.
...
\item[Propagation phase ($L/2$ ticks):]
  A Bresenham wavefront propagates from the centre row (or column)
  outward, one cell per tick.  Each cell reads its inward
  neighbour's accumulator and applies a single add-compare step.
  This is structurally identical to the BFS wavefront in our
  pulsating-shell CA~\cite{bessa2025sinc}.
\item[Shift phase ($\le L/2$ ticks):]
  ...
  lateral neighbour and decrements.  Rows that finish early are inert.
\end{description}
...
All experiments use a centred cube of side $L/2$ with a small
asymmetric flag (3 cells protruding in $+x$) as a fiducial marker.
The rotation angle is $\theta=30^\circ$, specified by $(p,q)=(1,2)$.
...
total number of active cells is preserved \emph{exactly} for any angle
and any~$L$.
...
\item \textbf{Local neighbourhood.}
  During the propagation phase, each cell reads one neighbour
  (toward the centre) for the accumulator.  During the shift phase,
  each cell reads one lateral neighbour for the content.
...
\bibitem{bessa2025sinc}
A.~Bessa,
``Integer sinc-wave cellular automaton with pulsating wavefront
and cylindrical helix,''
